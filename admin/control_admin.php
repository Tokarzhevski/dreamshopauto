...
if (isset($_SESSION['adminid']) && isset($_SESSION['adminlogin']) && isset($_SESSION['adminawatar']) && isset($_SESSION['admincont'])){
    ...
    <div class="site-wrap">
        ...
        <div class='col-md-8 ml-auto mr-auto mb-1 text-center forma'>
<?
$query="Select id, login, awatar, contact from admin";
$rezult=mysqli_query($dbc, $query) or die ("error");
echo "<table class='table table-light h4'>
...
while ($next=(mysqli_fetch_array($rezult))){
    ...
    echo "<thead class='text-info'>
              <tr>
                <td>$login</td>
                <td><img src='image/".$awatar."' width='100px' height='100px'></td>
                <td>$contact</td>
                <td><a href='del_admin.php?id=".$id."&login=".$login."' class='h1 text-danger'><span class='icon icon-delete'></span></a></td>
                <td><a href='update_admin.php?id=".$id."&login=".$login."' class='h1 text-success'><span class='icon icon-mode_edit '></span></a></td></tr></thead>";
}
echo "</table>";
mysqli_close($dbc);
?>
